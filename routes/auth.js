...
router.get("/trainers/:id", async (req, res) => {
  try {
    ...
    // ✅ Thêm domain cho ảnh
    if (trainer.image && !trainer.image.startsWith("http")) {
      trainer.image = `http://localhost:5000${trainer.image}`;
    }

    res.json(trainer);
  } catch (err) {
    console.error("❌ Lỗi lấy trainer:", err);
    res.status(500).json({ message: "Lỗi server", error: err.message });
  }
});
...
router.put("/profile", authenticate, async (req, res) => {
  try {
    const {
      ...
      phone,
      dateOfBirth,
      healthInfo: { gender, height, weight } = {}, // 🩵 fix tại đây
    } = req.body;
    const userId = req.user._id;
    const HealthInfo = require("../models/HealthInfo");
    ...
    res.json({
      message: "Cập nhật profile thành công",
      data: { user: updatedUser, healthInfo },
    });
  } catch (error) {
    console.error("Update profile error:", error);
    res.status(500).json({ message: "Lỗi server", error: error.message });
